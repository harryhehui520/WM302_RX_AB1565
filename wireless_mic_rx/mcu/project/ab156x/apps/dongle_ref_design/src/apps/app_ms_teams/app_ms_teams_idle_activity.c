    ...
    switch (event_group) {
            ...
            if (event_id == BT_GAP_LE_READ_RSSI_CNF) {
                ...
                if (bt_event_data && bt_event_data->buffer) {
                    ...
                    if (read_rssi_result->rssi < 100) {
                        ...
                        telemetry_info->radio_link_quality = 0x02;
                        ms_teams_telemetry_report_uint8_value(MS_TEAMS_TELEMETRY_RADIO_LINK_QUALITY, 0x2, false);
                    }
                }
                ...
                if (teams_event == MS_TEAMS_EVENT_VER_SYNC) {
                    ...
                    for (; group_idx < APP_LE_AUDIO_UCST_LINK_MAX_NUM; group_idx++) {
                        ...
                        if (link_info && link_info->group_id != s_last_group_id) {
                            ms_teams_telemetry_report_uint8_value(MS_TEAMS_TELEMETRY_WIREL_DEVICE_CHANGE, 0x01, false);
                    ...
                    if (group_idx >= APP_LE_AUDIO_UCST_LINK_MAX_NUM) {
                        ms_teams_telemetry_report_uint8_value(MS_TEAMS_TELEMETRY_WIREL_DEVICE_CHANGE, 0x00, false);
                    }
#endif
#else
                    ...
                    int32_t *sta_info = (int32_t *)pkg->data;
                    static uint8_t sidetone_level[16];
                    snprintf((char*)&sidetone_level[0], 16, "%d%c", (int)*sta_info, 0);
                    app_teams_telemetry_info *telemetry_info = app_ms_teams_get_telemetry_info();
                    telemetry_info->side_tone_level = &sidetone_level[0];
                } else if (teams_event == MS_TEAMS_EVENT_DSP_EFFECT_UPDATE) {
                    uint32_t effects = *(uint32_t *)pkg->data;
                    app_teams_telemetry_info *telemetry_info = app_ms_teams_get_telemetry_info();
                    telemetry_info->dsp_effect = effects;
                    ms_teams_telemetry_report_uint32_value(MS_TEAMS_TELEMETRY_DSP_EFFECTS_ENABLED, effects, true);
                    ...
                    ms_teams_telemetry_report_uint8_value(MS_TEAMS_TELEMETRY_VOICE_MUTE_ACTIVITY, *sta > 0 ? 0x1 : 0, true);
                }
        ...
        case EVENT_GROUP_UI_SHELL_APP_INTERACTION: {
            ...
            if (event_id == APPS_EVENTS_INTERACTION_UPDATE_LED_BG_PATTERN) {
                ...
                send_data[1] = 0x0;
                ms_teams_porting_usb_tx(send_data, 2);
            } else if (event_id == APPS_EVENTS_INTERACTION_TEAMS_DELAY_REPORT_BTN_PRESS_INFO) {
                uint16_t telemetry_data = (uint16_t)(uint32_t)extra_data;
                ms_teams_telemetry_report_uint16_value(MS_TEAMS_TELEMETRY_BTN_PRESS_INFO, telemetry_data, true);
                ...
                if (telemetry_info->radio_link_quality != temp_quality) {
                    telemetry_info->radio_link_quality = temp_quality;
                    ms_teams_telemetry_report_uint8_value(MS_TEAMS_TELEMETRY_RADIO_LINK_QUALITY, telemetry_info->radio_link_quality, true);
                }
...
{
    ui_shell_remove_event(EVENT_GROUP_UI_SHELL_APP_INTERACTION, APPS_EVENTS_INTERACTION_TEAMS_DELAY_REPORT_BTN_PRESS_INFO);
    uint16_t data = type << 8;
    if (press) {
        data |= 0x1;
    }
    ui_shell_send_event(false, EVENT_PRIORITY_HIGHEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
                                APPS_EVENTS_INTERACTION_TEAMS_DELAY_REPORT_BTN_PRESS_INFO, (void*)(uint32_t)data, 0,
                                NULL, 50);
}
