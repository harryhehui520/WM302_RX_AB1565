...
static void app_bt_source_handle_usb_streaming(bt_source_srv_port_t port, bool enable)
{
    ...
    if (enable) {
        APPS_LOG_MSGID_W(LOG_TAG" handle_usb_streaming, AUDIO_PLAY port=%d exist_call=%d", 2, port, exist_call);
    } else {
        APPS_LOG_MSGID_W(LOG_TAG" handle_usb_streaming, AUDIO_STOP port=%d exist_call=%d", 2, port, exist_call);
    }
...
{
    if (event_id == APPS_EVENTS_I2S_IN_STATUS_CHANGE && extra_data != NULL) {
...
{
    if (event_id == APPS_EVENTS_INTERACTION_LINE_IN_STATUS && extra_data != NULL) {
        bool line_in = (bool)extra_data;
        app_bt_source_ctx.line_in_port_enable = line_in;
//        if (app_bt_source_conn_mgr_is_disabled()) {
//            APPS_LOG_MSGID_E(LOG_TAG" LINE_IN event, line_in=%d but BT Source disabled",
//                             1, line_in);
//            return FALSE;
//        }

        bt_source_srv_audio_port_t port_param = {0};
        port_param.port = BT_SOURCE_SRV_PORT_LINE_IN;
        port_param.state = (line_in ? BT_SOURCE_SRV_AUDIO_PORT_STATE_ENABLE : BT_SOURCE_SRV_AUDIO_PORT_STATE_DISABLE);
        bool success = app_bt_source_send_action(BT_SOURCE_SRV_ACTION_AUDIO_PORT, &port_param, sizeof(port_param));
        APPS_LOG_MSGID_W(LOG_TAG" LINE_IN event, line_in=%d success=%d", 2, line_in, success);
    }
    return FALSE;
}
#endif
