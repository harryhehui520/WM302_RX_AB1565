...
bool app_ull_dongle_le_sirk_is_null(void)
{
...
// copy from app_ull_dongle_le_is_ull_adv
static bool arc_app_ull_dongle_le_is_pairing_mode_adv(bt_gap_le_ext_advertising_report_ind_t *adv_report)
{
    bt_bd_addr_t empty_addr = {0};
    uint16_t i = 0;

    if (!(adv_report->event_type & BT_GAP_LE_EXT_ADV_REPORT_EVT_MASK_CONNECTABLE)) {
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"[APP][U] arc_app_ull_dongle_le_is_pairing_mode_adv, adv not CONNECTABLE", 0);
        return false;
    }
    ...
    if ((!(adv_report->event_type & BT_GAP_LE_EXT_ADV_REPORT_EVT_MASK_DIRECTED) &&
        ...
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"[APP][U] arc_app_ull_dongle_le_is_pairing_mode_adv, adv addr is empty", 0);
        return false;
    }
        ...
        if (adv_report->data[i] >= 3) {
            if ((adv_report->data[i + 1] == BT_GAP_LE_AD_TYPE_MANUFACTURER_SPECIFIC) \
              ...
              && (adv_report->data[i + 3] == ((APP_DONGLE_CM_PAIRING_MODE_ADV_DATA & 0xFF00)) >> 8)) {
                 APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"[APP][U] PAIRING MODE ADV!!", 0);
                 return TRUE;
             }
        }
...
static bool app_ull_dongle_le_is_ull_adv(bt_gap_le_ext_advertising_report_ind_t *adv_report)
{
    ...
    if (!(adv_report->event_type & BT_GAP_LE_EXT_ADV_REPORT_EVT_MASK_CONNECTABLE)) {
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"[APP][U] app_ull_dongle_le_is_ull_adv, adv not CONNECTABLE", 0);
        return false;
    }
    ...
    if ((!(adv_report->event_type & BT_GAP_LE_EXT_ADV_REPORT_EVT_MASK_DIRECTED) &&
        ...
        ((adv_report->event_type & BT_GAP_LE_EXT_ADV_REPORT_EVT_MASK_DIRECTED) &&
         (0 == memcmp(adv_report->direct_address.addr, empty_addr, sizeof(bt_bd_addr_t))))) {
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"[APP][U] app_ull_dongle_le_is_ull_adv, adv addr is empty", 0);
        return false;
    }
...
static void app_ull_dongle_le_handle_adv_report_ind(bt_status_t status, bt_gap_le_ext_advertising_report_ind_t *ind)
{
    if (APP_ULL_DONGLE_LE_STATE_ENABLED != g_app_ull_le_ctrl.ull_state) {
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"adv report fail, ull_state is 0x%x!", 1, g_app_ull_le_ctrl.ull_state);
        return;
    }
    ...
    if (!app_ull_dongle_le_is_ull_adv(ind)) {
        APPS_LOG_MSGID_W(APP_ULL_LOG_TAG"adv report fail, not ull adv!", 0);
        return;
    }
    ...
    switch (g_app_ull_le_ctrl.curr_conn) {
        case APP_ULL_DONGLE_LE_CONN_BONDED_DEVICE:
        case APP_ULL_DONGLE_LE_CONN_NEW_DEVICE: {
            if (APP_ULL_DONGLE_LE_CONN_NEW_DEVICE == g_app_ull_le_ctrl.curr_conn && !arc_app_ull_dongle_le_is_pairing_mode_adv(ind))
            {
                APPS_LOG_MSGID_W(APP_ULL_LOG_TAG"adv report fail, curr_conn is new device but remote not in pairing mode adv!", 0);
                return;
            }

            if (APP_ULL_DONGLE_LE_CONN_BONDED_DEVICE == g_app_ull_le_ctrl.curr_conn && arc_app_ull_dongle_le_is_pairing_mode_adv(ind))
            {
                APPS_LOG_MSGID_W(APP_ULL_LOG_TAG"adv report fail, curr_conn is bonded device but remote in pairing mode adv!", 0);
                return;
            }
...
/* HWJ: Risk in global variable sharing? Same in ui_shell, no problem. */
extern bool is_in_pairing;
#define APP_DONGLE_CM_TIMER_ID_USER_PAIRING_END     0x04
extern void app_dongle_cm_stop_timer(uint8_t timer_id);
...
static void app_ull_dongle_le_handle_ull_connect_ind(bt_ull_le_connected_info_t *ind)
{
    ...
    bool air_pairing = arc_app_dongle_cm_pairing_mode_is_enable();

    APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"Group device addr:type: %d, is pairing: %d, ull_state: %d, conn handle: %x, addr: %x-%x-%x-%x-%x-%x", 10, ind->group_device_addr[0].type,
        air_pairing,
    ...
    if (BT_STATUS_SUCCESS == ind->status) {
        ...
        if ((!app_ull_dongle_le_sirk_is_null()) &&
            ...
            APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"Dongle and device SIRK is different!", 0);
            if (air_pairing || is_in_pairing) {
                bt_ull_change_remote_sirk_t sirk;
                sirk.handle = ind->conn_handle;
                memcpy((uint8_t *)&sirk.sirk, (uint8_t *)&g_app_ull_le_ctrl.sirk, sizeof(bt_key_t));
                bt_ull_le_srv_set_sirk(g_app_ull_le_ctrl.sirk);
                bt_ull_le_srv_action(BT_ULL_ACTION_CHANGE_REMOTE_SIRK, &sirk, sizeof(bt_ull_change_remote_sirk_t));
                if (0 != g_app_ull_le_bonded_info.num) {
                    uint8_t i = 0;
                    bt_addr_t *addr = app_ull_dongle_le_get_bt_addr_by_conn_handle(ind->conn_handle);
                    if (addr) {
                        for (i = 0; i < g_app_ull_le_bonded_info.num; i ++) {
                            /* HWJ: So far as I know, bonded info means that devices can be connected, it seems airoha do not want the other device
                                to be connected during pairing. but I want to keep the last connected device, so comment out here. */
                            #if 0
                            if (memcmp((uint8_t *)&addr->addr, &g_app_ull_le_bonded_info.addr[i], sizeof(bt_bd_addr_t))) {
                                bt_addr_t remove_addr;
                                remove_addr.type = app_ull_dongle_le_get_addr_type_from_bonding_list(&g_app_ull_le_bonded_info.addr[i]);
                                memcpy(&remove_addr.addr, &g_app_ull_le_bonded_info.addr[i], sizeof(bt_bd_addr_t));
                                app_ull_dongle_le_delete_device(&remove_addr);
                            }
                            #endif
                        }
                    }
        ...
        if ((BT_ULL_LE_MAX_LINK_NUM == link_num) || (g_app_ull_le_ctrl.set_size == link_num)) {
            ...
            app_dongle_cm_stop_timer(APP_DONGLE_CM_TIMER_ID_USER_PAIRING_END);
            is_in_pairing = FALSE;
        ...
        if (g_app_ull_le_ctrl.set_size != link_num) {
            ...
            if (is_in_pairing)
            {
                /* HWJ: We are in paring mode, scan new device anyway. */
                app_ull_dongle_le_scan_device(APP_ULL_DONGLE_LE_SCAN_NEW_DEVICE);
            }
            else
            {
                if (app_ull_dongle_le_sirk_is_null())
...
*/
    ...
    //}
    APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"[APP][U] LE_DISCONNECT_IND, Normal! handle:%x reason:%x, link num:%d, cs:%d, ct: %d, air pairing: %d", 6, \
        ind->connection_handle, ind->reason, link_num, g_app_ull_le_ctrl.set_size, g_app_ull_le_ctrl.client_type, air_pairing);
...
bt_status_t app_ull_dongle_le_start_scan(void)
{
    ...
    } else if (bt_device_manager_le_get_bonded_number() != 0) {
        /* Rx start scan bond device if bonded number not 0 */
        APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"scan bond devie", 0);
        return app_ull_dongle_le_scan_device(APP_ULL_DONGLE_LE_SCAN_BONDED_DEVICE);
    } else {
...
void app_ull_dongle_le_delete_device(bt_addr_t *addr)
{
    ...
    /* update bonded info */
    app_ull_dongle_le_update_bonded_info();
    APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"app_ull_dongle_le_delete_device done! addr: %x-%x-...%x", 3, addr->addr[0], addr->addr[1], addr->addr[5]);
}
