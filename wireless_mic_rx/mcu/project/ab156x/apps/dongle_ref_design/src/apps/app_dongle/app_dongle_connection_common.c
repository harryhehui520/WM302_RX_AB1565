...
#define APP_DONGLE_CM_MUTEX_LOCK() bt_os_take_stack_mutex()
#define APP_DONGLE_CM_MUTEX_UNLOCK() bt_os_give_stack_mutex()
...
static void arc_app_dongle_cm_user_pairing_end_timeout_handler(uint8_t timer_id, void *data);
...
#define APP_DONGLE_CM_TIMER_MAX     0x04
...
bt_status_t app_dongle_cm_init(void)
{
    ...
    app_dongle_cm_ctx.pairing_mode.pm_timeout = 120000;     // HWJ: default: 1min, Sunstone Air set to 2min.
...
#else /* !ARC_UNUSED_CODE_BTA_LEA */
        APPS_LOG_MSGID_E(APP_DONGLE_CM_LOG_TAG"app_dongle_cm_enter_le_pairing_mode. fail", 0);
        assert(0);
#endif /* ARC_UNUSED_CODE_BTA_LEA */
    }
...
bool arc_app_dongle_cm_pairing_mode_is_enable(void)
{
    app_dongle_cm_ctrl_contex_t *ctrl_ctx = app_dongle_cm_get_ctrl_context();
    return ctrl_ctx->pairing_mode.is_enter_pm;
}

bool is_in_pairing = FALSE;
...
void app_dongle_cm_pairing_key_event_handler(void)
{
    ...
    if (ctrl_ctx->link_mode & APP_DONGLE_CM_LINK_MODE_ULL_V2) {
        ...
        if (APP_DONGLE_CM_PAIRING_MODE_FEATURE_IS_ENABLE) {
            ...
            is_in_pairing = true;
            app_dongle_cm_start_timer(APP_DONGLE_CM_TIMER_ID_USER_PAIRING_END, ctrl_ctx->pairing_mode.pm_timeout, arc_app_dongle_cm_user_pairing_end_timeout_handler, NULL);
        } else {
#ifdef ARC_UNUSED_CODE_BTA_LEA
            if (ctrl_ctx->le_scan.use_sirk) {
...
#else /* !ARC_UNUSED_CODE_BTA_LEA */
            APPS_LOG_MSGID_E(APP_DONGLE_CM_LOG_TAG"app_dongle_cm_pairing_key_event_handler. fail", 0);
            assert(0);
#endif /* ARC_UNUSED_CODE_BTA_LEA */
        }
...
#ifdef ARC_UNUSED_CODE_BTA_LEA  // HWJ: do not connect unkonwn device.
        scan_type_le = APP_DONGLE_CM_CONNECTION_USING_FULL_SCAN;
#else
        if (ctrl_ctx->le_list.num != 0)
            scan_type_le = APP_DONGLE_CM_CONNECTION_USING_BONDING_LIST;
#endif /* ARC_UNUSED_CODE_BTA_LEA */
    }
...
static void arc_app_dongle_cm_user_pairing_end_timeout_handler(uint8_t timer_id, void *data)
{
    if (timer_id != APP_DONGLE_CM_TIMER_ID_USER_PAIRING_END) {
        return;
    }

    APPS_LOG_MSGID_I(APP_DONGLE_CM_LOG_TAG"arc_app_dongle_cm_user_pairing_end_timeout_handler", 0);

    is_in_pairing = false;

    ui_shell_send_event(FALSE, EVENT_PRIORITY_HIGH, EVENT_GROUP_BT_ULTRA_LOW_LATENCY, BT_ULL_EVENT_USER_PAIRING_END_TIMEOUT, NULL, 0, NULL, 0);
}
                ...
                if (source_info1->source_registed &&
                    ...
                    if (ctrl_ctx->le_scan.use_sirk \
                        ...
                        if (BT_STATUS_SUCCESS == (ret = source_info1->cm_handle.precheck((app_dongle_cm_precheck_data_t*)data))) {
                            APPS_LOG_MSGID_I(APP_DONGLE_CM_LOG_TAG"Precheck PASS(ULL v2 sirk)!!", 0);
                        ...
                        APPS_LOG_MSGID_I(APP_DONGLE_CM_LOG_TAG"Precheck PASS(ULL v2 normal)!!", 0);
                        ret = source_info1->cm_handle.start_source(data->address, param);
                    }
                    if (ret) {
            ...
            if (ctrl_ctx->link_mode & APP_DONGLE_CM_LINK_MODE_LEA && APP_DONGLE_CM_STATE_START_SCAN_INQUIRY == source_info_lea->conn_state) {
                ...
                if (source_info1->source_registed &&
                    ...
                    if (ctrl_ctx->le_scan.use_sirk \
                        && APP_DONGLE_CM_CONNECTION_USING_SIRK == ctrl_ctx->connect_style_le) {
                         if (BT_STATUS_SUCCESS == (ret = source_info1->cm_handle.precheck((app_dongle_cm_precheck_data_t*)data))) {
                             APPS_LOG_MSGID_I(APP_DONGLE_CM_LOG_TAG"Precheck PASS(LEA sirk)!!", 0);
                             ret = source_info1->cm_handle.start_source(data->address, param);
                         } else {
                             APPS_LOG_MSGID_E(APP_DONGLE_CM_LOG_TAG"Precheck FAIL(LEA)!!", 0);
                         }
                     } else {
                         APPS_LOG_MSGID_I(APP_DONGLE_CM_LOG_TAG"Precheck PASS(LEA normal)!!", 0);
                         ret = source_info1->cm_handle.start_source(data->address, param);
                     }
                    if (ret) {
