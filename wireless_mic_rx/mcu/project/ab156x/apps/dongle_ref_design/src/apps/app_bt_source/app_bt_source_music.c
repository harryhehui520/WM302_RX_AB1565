                                                     ...
                                                     size_t data_len)
{
    switch (event_id) {
        ...
        case BT_SOURCE_SRV_EVENT_MUSIC_STREAM_OPERAION_IND: {
        ...
        case BT_SOURCE_SRV_EVENT_MUSIC_DETECT_MEDIA_DATA_IND: {
            ...
            if (event == BT_SOURCE_SRV_MUSIC_DATA_SUPNED || event == BT_SOURCE_SRV_MUSIC_DATA_RESUME) {
                memcpy(&app_bt_source_music_ctx.peer_address, peer_addr, sizeof(bt_addr_t));

                bt_source_srv_music_action_t music_action = {0};
                if (event == BT_SOURCE_SRV_MUSIC_DATA_SUPNED) {
                    music_action.data = &data_detect;
                }
                memcpy(&music_action.peer_address, peer_addr, sizeof(bt_addr_t));
                bt_status = bt_source_srv_send_action((event == BT_SOURCE_SRV_MUSIC_DATA_SUPNED ? BT_SOURCE_SRV_ACTION_SUSPEND_STREAM : BT_SOURCE_SRV_ACTION_START_STREAM),
                                                      &music_action, sizeof(music_action));
            }
