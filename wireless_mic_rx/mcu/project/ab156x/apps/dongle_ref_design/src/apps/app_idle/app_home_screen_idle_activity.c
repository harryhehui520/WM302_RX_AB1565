...
#define WAIT_TIME_BEFORE_POWER_OFF  (500)              /* The delay time to do system power off for playing VP and LED. */
...
void app_home_screen_set_ready_to_off(void)
{
    s_ready_to_off = true;
}
...
static bool ARC_s_powering_on = FALSE;
                                  ...
                                  size_t data_len)
{
...
#ifdef ARC_DISABLE_AUTO_POWER_ON
    if (action == KEY_POWER_ON)
    {
        if (local_context->bt_power_state == APP_HOME_SCREEN_BT_POWER_STATE_DISABLED)
        {
            ARC_s_powering_on = TRUE;
        }
    }
    else
    {
        if (ARC_s_powering_on)
        {
            /* Assuming pairing action follows immediately after power-on, if not, end anyway. */
            ARC_s_powering_on = FALSE;

            if (KEY_DONGLE_COMMON_PAIRING_FROM_POWER_ON == action)
            {
                APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY", KEY_DONGLE_COMMON_PAIRING_FROM_POWER_ON", 0);

                /* Do KEY_DONGLE_COMMON_PAIRING */
                uint16_t *p_key_action = (uint16_t *)pvPortMalloc(sizeof(uint16_t));
                if (p_key_action) {
                    *p_key_action = KEY_DONGLE_COMMON_PAIRING;
    ...
    switch (action) {
        ...
        case KEY_POWER_OFF:
            ...
            break;
        case KEY_POWER_ON:
            // HWJ: Power on cmd received, cancel pending timeout.
            ui_shell_remove_event(EVENT_GROUP_UI_SHELL_APP_NXP_COM, SMCHARGER_EVENT_POWERING_ON_TIMEOUT);
