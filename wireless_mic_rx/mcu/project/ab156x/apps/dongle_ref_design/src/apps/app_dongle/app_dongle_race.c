...
void arc_apps_dongle_race_cmd_get_addr_from_channel_id(bt_addr_t *bt_addr, uint8_t channel_id)
{
    bt_bd_addr_t *addr = race_get_bt_connection_addr(channel_id);

    if (addr)
    {
        memcpy(&bt_addr->addr, addr, sizeof(bt_bd_addr_t));
        bt_addr->type = BT_ADDR_RANDOM;    // HWJ: Set BT_ADDR_PUBLIC from apps_dongle_race_cmd_on_remote_control_state_change, but BT_ADDR_RANDOM is actually.
    }
    else
    {
        APPS_LOG_MSGID_E(LOG_TAG"arc_apps_dongle_race_cmd_get_addr_from_channel_id fail.", 0);
    }
}
...
extern bool arc_dp_recored_update_display_record_rssi(bt_addr_t *addr, int8_t rssi);
    ...
    if (BT_GAP_LE_READ_RSSI_CNF == event_id && extra_data) {
        ...
        if (bt_event_data->buffer) {
            ...
            if (addr) {
                ...
                if (BT_HCI_STATUS_SUCCESS == read_rssi_result->status)
                {
                    arc_dp_recored_update_display_record_rssi(addr, read_rssi_result->rssi);
                }
                else
                {
                    arc_dp_recored_update_display_record_rssi(addr, -99);    // HWJ: Indicate error by reporting fixed number.
                }
            }
            APPS_LOG_MSGID_I(LOG_TAG"notify handle = %d rssi= %d resule: 0x%X.", 3, read_rssi_result->handle, read_rssi_result->rssi, race_notify_ret);
        }
    }
#endif
    return ret;
