...
bt_status_t app_le_audio_init_audio_transmitter(app_le_audio_stream_port_t port)
{
    ...
    if (APP_LE_AUDIO_MODE_UCST == g_lea_ctrl.curr_mode) {
        ...
        if (APP_LE_AUDIO_STREAM_PORT_MIC_0 == port) {
            p_codec_state = app_le_audio_ucst_get_ase_codec_config(context_type, AUDIO_DIRECTION_SOURCE);
            if (NULL == p_codec_state) {
                return BT_STATUS_FAIL;
            }
            audio_location_channels = app_le_audio_ucst_get_location_count(p_codec_state->audio_channel_allocation);
            ...
            frame_duration_source = p_codec_state->frame_duration ? 10000 : 7500;

            framesize_source = p_codec_state->codec_frame_blocks_per_sdu * audio_location_channels * p_codec_state->octets_per_codec_frame;
            bitrate_source = (framesize_source * 8 * 1000 * 1000) / frame_duration_source;
            sampling_freq_source =  app_le_audio_convert_sample_freq(p_codec_state->sampling_frequency);
        }
    }
/*
