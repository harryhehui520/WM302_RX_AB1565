...
static bool app_ms_teams_idle_teams_ev_proc(struct _ui_shell_activity *self, uint32_t ev, uint8_t *data, uint32_t data_len)
{
    ...
    switch (event) {
        ...
        case MS_TEAMS_DISCONNECTED: {
            ...
            APPS_LOG_MSGID_I(TAG"dis_ch=%d disconnected.", 1, dis_ch->channel_type);
...
static bool _proc_key_action(apps_config_key_action_t action, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    switch (action) {
        ...
        case KEY_MS_TEAMS_BTN_INVOKE: {
            ...
            ret = true;
            if (s_ctx.connected_clients[client_idx].channel_type == MS_TEAMS_LINK_CHANNEL_GATT) {
                break;
            }
        ...
        case KEY_MS_TEAMS_BTN_RELEASE: {
            ...
            if (s_ctx.connected_clients[client_idx].channel_type == MS_TEAMS_LINK_CHANNEL_RACE) {
                apps_dongle_sync_event_send_extra_by_address(EVENT_GROUP_UI_SHELL_KEY, event_id, (void *)&action, sizeof(apps_config_key_action_t), &s_ctx.connected_clients[client_idx].address);
            } else {
        ...
        case EVENT_GROUP_UI_SHELL_AUDIO_ANC: {
            if (event_id == AUDIO_ANC_CONTROL_EVENT_ON
                ...
                apps_dongle_sync_event_send_extra(EVENT_GROUP_UI_SHELL_MS_TEAMS, (MS_TEAMS_EVENT_DSP_EFFECT_UPDATE << 16) & 0xFFFF0000,
                                                          &info->dsp_effect, sizeof(info->dsp_effect));
            }
            break;
        }
#endif
            ...
            if (event_id == APPS_EVENTS_INTERACTION_UPDATE_LED_BG_PATTERN) {
...
#ifdef AIR_USB_HID_CALL_CTRL_ENABLE
                if (s_ctx.last_hid_call_state_event != USB_HID_SRV_EVENT_CALL_END && s_ctx.last_hid_call_state_event != USB_HID_SRV_EVENT_CALL_MAX) {
                    APPS_LOG_MSGID_I(TAG"APPS_EVENTS_INTERACTION_UPDATE_LED_BG_PATTERN received, call state=%d.", 1, s_ctx.last_hid_call_state_event);
                    switch (s_ctx.last_hid_call_state_event) {
                ...
                if (s_ctx.last_hid_call_state_event != USB_HID_SRV_EVENT_CALL_MAX) {
...
#endif
                else if (s_ctx.last_notify_ev != MS_TEAMS_NOTIF_EVENT_NONE) {
                    if (s_ctx.last_notify_ev == MS_TEAMS_NOTIF_EVENT_UPCOMING_SCHEDULED_MEETING) {
                        /* todo */
                    }
                    APPS_LOG_MSGID_I(TAG"APPS_EVENTS_INTERACTION_UPDATE_LED_BG_PATTERN received, but notify sta still is %d.", 1, s_ctx.last_notify_ev);
                    /* Use the incoming call's led UI to indicate these event. */
                    apps_config_set_background_led_pattern(LED_INDEX_INCOMING_CALL, true, APPS_CONFIG_LED_AWS_SYNC_PRIO_MIDDLE);
                    ret = true;
                } else {
                    if (s_ctx.teams_connected) {
                        ...
                        if ((time - s_ctx.usb_connected_time) >= 10) {
...
#ifdef AIR_USB_HID_CALL_CTRL_ENABLE
                ms_teams_telemetry_report_uint8_value(MS_TEAMS_TELEMETRY_VOICE_MUTE_ACTIVITY, det > 0 ? 1 : 0, true);
#endif
                ...
                apps_dongle_sync_event_send_extra(EVENT_GROUP_UI_SHELL_MS_TEAMS, (MS_TEAMS_EVENT_IN_EAR_STA << 16) & 0xFFFF0000,
                                                          sta_info, sizeof(app_in_ear_sta_info_t));
