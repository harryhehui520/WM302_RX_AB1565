        ...
        if (type == LLF_TYPE_HEARING_AID) {
            if (event == LLF_CONTROL_EVENT_ON) {
                ...
                if (app_hear_through_ctx.is_anc_changed == true) {
                    ...
                    if (app_hearing_aid_is_mp_test_mode() == false) {
...
static bool app_hear_through_activity_handle_vp_streaming_end(void *extra_data, uint32_t extra_data_len)
{
    ...
    APPS_LOG_MSGID_I(APP_HEAR_THROUGH_ACT_TAG"[app_hear_through_activity_handle_vp_streaming_end] power_on_vp_played : %d, charger_in : %d, powering_off : %d",
                     3,
                     app_hear_through_ctx.is_power_on_vp_played,
...
static bool app_hear_through_activity_handle_aws_config_sync(void *extra_data, uint32_t extra_data_len)
{
    ...
    APPS_LOG_MSGID_I(APP_HEAR_THROUGH_ACT_TAG"[app_hear_through_activity_handle_aws_config_sync] CONFIGURATION_SYNC event, is_key_event : %d, hear_through_switch : %d, hear_through_mode : %d, ambient_control_index : %d, is_anc_changed : %d",
                        5,
                                                    ...
                                                    size_t data_len)
{
    ...
    if (status == BT_DEVICE_MANAGER_POWER_STATUS_SUCCESS) {
        if (evt == BT_DEVICE_MANAGER_POWER_EVT_ACTIVE_COMPLETE) {
            APPS_LOG_MSGID_I(APP_HEAR_THROUGH_ACT_TAG"[app_hear_through_activity_proc_dm_event] BT Powering ON, is_charger_in : %d, power_on_vp_played : %d",
                                3,
                                app_hear_through_ctx.is_charger_in,
                                app_hear_through_ctx.is_hear_through_enabled,
                                app_hear_through_ctx.is_power_on_vp_played);
...
{
    APPS_LOG_MSGID_I(APP_HEAR_THROUGH_ACT_TAG"[app_hear_through_handle_charger_out] Charger OUT, power_on_vp_played : %d",
                        1,
                        app_hear_through_ctx.is_power_on_vp_played);
    ...
    /**
     * @brief Fix issue that if the power on VP is not played when out of case.
     * need to play the power on VP.
     */
    // if (app_hear_through_ctx.is_power_on_vp_played == false) {
        // voice_prompt_play_vp_power_on();
    // } else {
