...
#elif defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)     // Only ULL2 Enable
...
{
    APPS_LOG_MSGID_W(LOG_TAG"[SUB_MODE] update_multi_adv, sub_mode_bitmask=0x%04X sub_mode_num=%d index=%d sub_mode=%d",
                     4, app_lea_adv_mgr_ctx.sub_mode_bitmask, app_lea_adv_mgr_ctx.sub_mode_num,
...
static void app_lea_adv_mgr_do_start_advertising(uint8_t mode, uint32_t timeout)
{
...
#if 0//def AIR_SMART_CHARGER_ENABLE
    ...
    if (app_speaker_is_switch_mode_ongoing()) {
        ...
        APPS_LOG_MSGID_E(LOG_TAG" do_start_advertising, SPEAKER_MODE_SWITCHING", 0);
    } else if (aws_mode == BT_AWS_MCE_SRV_MODE_BROADCAST) {
        error_code = APP_LEA_ADV_ERROR_SPEAKER_BROADCAST_MODE;
        APPS_LOG_MSGID_E(LOG_TAG" do_start_advertising, SPEAKER_BROADCAST_MODE", 0);
    }
    ...
    // Disable LEA ADV (CIS) on double mode
        ...
        APPS_LOG_MSGID_E(LOG_TAG" do_start_advertising, SPEAKER_DOUBLE_MODE", 0);
    }
#endif
#endif
