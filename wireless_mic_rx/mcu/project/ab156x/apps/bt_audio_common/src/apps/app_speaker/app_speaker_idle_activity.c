...
static void app_speaker_control_music_play(bt_aws_mce_srv_mode_t pre_mode, bt_aws_mce_role_t pre_role)
{
    ...
    if (role == BT_AWS_MCE_ROLE_AGENT) {
        if ((pre_mode == BT_AWS_MCE_SRV_MODE_SINGLE || pre_role != BT_AWS_MCE_ROLE_AGENT) && mode == BT_AWS_MCE_SRV_MODE_BROADCAST) {
    ...
    switch (event_group) {
        ...
        case EVENT_GROUP_UI_SHELL_APP_INTERACTION:
            if (event_id == APPS_EVENTS_INTERACTION_USB_AUDIO_OP) {
                if (app_usb_in_is_open() && (bt_aws_mce_srv_get_mode() != BT_AWS_MCE_SRV_MODE_SINGLE)
                    && (bt_aws_mce_srv_get_mode() != BT_AWS_MCE_SRV_MODE_NORMAL)) {
