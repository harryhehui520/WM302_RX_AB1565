...
{
    if (bond_info_order[0] == BT_DM_LE_INVAILD_INDEX)
    {
        LOG_MSGID_I(BT_DM, "[DM][LE] bt_device_manager_le_get_latest_bonded_info_index is empty", 0);
        return BT_DM_LE_INVAILD_INDEX;
    }

    int i;
    for (i = 1; i < BT_DEVICE_MANAGER_LE_BONDED_ORDER_MAX; i++){
        if (bond_info_order[i] == 0xff) {
    ...
    LOG_MSGID_I(BT_DM, "[DM][LE] bt_device_manager_le_get_latest_bonded_info_index, index = %d", 1, bond_info_order[i-1]);
    return bond_info_order[BT_DEVICE_MANAGER_LE_BONDED_ORDER_MAX-1];
}

static uint8_t bt_device_manager_le_get_second_latest_bonded_info_index(void)
{
    if (bond_info_order[0] == BT_DM_LE_INVAILD_INDEX)
    {
        LOG_MSGID_I(BT_DM, "[DM][LE] bt_device_manager_le_get_second_latest_bonded_info_index is empty", 0);
        return BT_DM_LE_INVAILD_INDEX;
    }

    if (bond_info_order[1] == BT_DM_LE_INVAILD_INDEX)
    {
        LOG_MSGID_I(BT_DM, "[DM][LE] bt_device_manager_le_get_second_latest_bonded_info_index index only one", 0);
        return BT_DM_LE_INVAILD_INDEX;
    }

    int i;
    for (i = 2; i < BT_DEVICE_MANAGER_LE_BONDED_ORDER_MAX; i++){
        if (bond_info_order[i] == BT_DM_LE_INVAILD_INDEX) {
            LOG_MSGID_I(BT_DM, "[DM][LE] bt_device_manager_le_get_second_latest_bonded_info_index, index = %d", 1, bond_info_order[i-2]);
            return bond_info_order[i-2];
        }
    }

    LOG_MSGID_I(BT_DM, "[DM][LE] bt_device_manager_le_get_second_latest_bonded_info_index, index = %d", 1, bond_info_order[i-2]);
    return bond_info_order[BT_DEVICE_MANAGER_LE_BONDED_ORDER_MAX-2];
}
...
static bt_dm_le_bond_info_context_t *bt_device_manager_le_find_old_bond_info_context(void)
{
    ...
    /* HWJ: May not need as AIROHA handle in the caller side */
    if (BT_DM_LE_INVAILD_INDEX == oldest_index)
    {
        return NULL;
    }
...
static bt_dm_le_bond_info_context_t *bt_device_manager_le_find_latest_bond_info_context(void)
{
    uint8_t index;
    index = bt_device_manager_le_get_latest_bonded_info_index();

    if (BT_DM_LE_INVAILD_INDEX == index)
    {
        return NULL;
    }

    return &bond_info_context[index];
}

static bt_dm_le_bond_info_context_t *bt_device_manager_le_find_second_latest_bond_info_context(void)
{
    uint8_t index;
    index = bt_device_manager_le_get_second_latest_bonded_info_index();
...
bt_device_manager_le_bonded_info_t *bt_device_manager_le_get_latest_bonded_info(void)
{
    if (!bt_dm_le_initialized) {
        LOG_MSGID_I(BT_DM, "[DM][LE] Device bonded status check fail, please init BT DM LE first!", 0);
        return NULL;
    ...
    bt_dm_le_bond_info_context_t *context = NULL;
    context = bt_device_manager_le_find_latest_bond_info_context();
    if (context != NULL) {
        return &(context->bonded_info);
    }
    return NULL;
...
bt_device_manager_le_bonded_info_t *bt_device_manager_le_get_second_latest_bonded_info(void)
{
    if (!bt_dm_le_initialized) {
        LOG_MSGID_I(BT_DM, "[DM][LE] Device bonded status check fail, please init BT DM LE first!", 0);
        return NULL;
    ...
    bt_dm_le_bond_info_context_t *context = NULL;
    context = bt_device_manager_le_find_second_latest_bond_info_context();
    if (context != NULL) {
        return &(context->bonded_info);
    }
    return NULL;
