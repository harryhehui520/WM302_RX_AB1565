...
TaskHandle_t battery_lineat_task_t = NULL;                                                       /*Task : create regular task for battery_management_gauge_get_vbat*/
...
int32_t battery_management_get_battery_property_internal(battery_property_t property)
{
    ...
    switch (property)
    {
        case BATTERY_PROPERTY_CAPACITY:
            property_value = battery_management_gauge_get_percent();
            break;
        case BATTERY_PROPERTY_CAPACITY_LEVEL :
            property_value = (battery_management_gauge_get_percent() / 10);
            break;
        ...
        case BATTERY_PROPERTY_TEMPERATURE:
            ...
            break;
        case BATTERY_PROPERTY_TEMPERATURE_VOLTAGE:
            if (battery_sw_ntc_feature == BATTERY_OPERATE_ON)
            {
                property_value = g_ntc_radio;
            }
            else
            {
                property_value = -1;
        ...
        case BATTERY_PROPERTY_VOLTAGE:
            property_value = battery_management_gauge_get_vbat();
            break;
...
#endif /* MTK_BATTERY_EOC_ENTER_RTC */
...
{
    xTaskCreate(battery_management_charger_task, "charger_task", 384, NULL, TASK_PRIORITY_NORMAL, &battery_regular_task_t);
#ifdef BATTERY_LINEAR_GAUGE
    xTaskCreate(battery_management_gauge_linear_task, "Linear_task", 384, NULL, TASK_PRIORITY_NORMAL, &battery_lineat_task_t);
#endif
...
battery_management_status_t battery_management_init_internal(void)
{
    ...
    battery_timer_init();
    battery_management_gauge_init();

#ifdef BATTERY_CHECK_BATTERY_VOLTAGE
    if(pmu_get_usb_input_status() != true)
    {
        uint32_t batsns = pmu_bat_get_pure_volt();
        LOG_MSGID_I(battery_management, "Battery voltage %d", 1, batsns);
        if (bm_cust.initial_bat > batsns ||  bm_cust.shutdown_bat > batsns)
        {
            LOG_MSGID_I(battery_management, "Battery voltage low\r\n", 0);
            hal_rtc_enter_rtc_mode();
        }
    }
#endif /* BATTERY_CHECK_BATTERY_VOLTAGE */
