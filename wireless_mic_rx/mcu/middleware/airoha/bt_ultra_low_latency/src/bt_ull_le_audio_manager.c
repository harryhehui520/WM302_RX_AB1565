...
static void bt_ull_le_am_dl_callback(bt_sink_srv_am_id_t aud_id, bt_sink_srv_am_cb_msg_class_t msg_id, bt_sink_srv_am_cb_sub_msg_t sub_msg, void *param);
...
static void bt_ull_le_am_dl_init(void)
{
    ...
    if (AUD_ID_INVALID ==(ull_dl_am_info->audio_id = bt_sink_srv_ami_audio_open(AUD_MIDDLE, bt_ull_le_am_dl_callback))) {
        ...
        if (ull_am_info->am_handle && (ull_am_info->am_handle == handle)) {
            if ((BT_ULL_LE_SUB_STATE_NONE == substate) || (BT_ULL_LE_SUB_STATE_PREPARE_AUDIO_SRC == substate)) {
              ...
              //ull_report("[ULL][LE][AM] dl play ind, init am codec parameter and audio play", 0);
                ...
                aud_cap.codec.ull_ble_dl_format.ble_codec.sample_rate = bt_ull_le_srv_get_codec_sample_rate(0, false);
                ...
                uint16_t frame_size = bt_ull_le_srv_get_sdu_size(false);
...
static bt_status_t bt_ull_le_am_ul_play(bt_sink_srv_am_id_t aud_id, bt_ull_le_codec_t codec_type)
{
    ...
    if (BT_ULL_MIC_CLIENT == client_type) {
        ...
        /*wireless mic*/
        aud_cap.codec.ull_ble_ul_format.ble_codec.sample_rate = bt_ull_le_srv_get_codec_sample_rate(2, true);
        aud_cap.codec.ull_ble_ul_format.ble_codec.channel_num = bt_ull_le_srv_get_channel_mode(2, true);
        aud_cap.codec.ull_ble_ul_format.ble_codec.frame_payload_length = bt_ull_le_srv_get_sdu_size(true);
        aud_cap.codec.ull_ble_ul_format.ble_codec.context_type = AUDIO_CONTENT_TYPE_WIRELESS_MIC;
        uint32_t sdu_interval = bt_ull_le_srv_get_sdu_interval(true);
...
static void bt_ull_le_am_ul_handle_stop_success_or_start_fail_cnf(bt_ull_le_am_ul_info_t* ull_am_info)
{
    ull_report("[ULL][LE][AM] handle ul stop success or start fail cnf, is_request_transmitter_start = 0x%x, flag:0x%x", 2, 
        ull_am_info->is_request_transmitter_start, ull_am_info->streaming_flag);
    bt_ull_le_remove_am_ul_flag(BT_ULL_LE_AM_UL_STARTING);
    bt_ull_le_remove_am_ul_flag(BT_ULL_LE_AM_UL_STOPPING);
...
bt_status_t bt_ull_le_am_set_mute(bt_ull_le_am_mode_t mode, bool is_mute)
{
    ...
    if (BT_ULL_LE_AM_DL_MODE == mode) {
        ...
        } else if (ull_dl_am_info->is_start_request && (!is_mute) && ull_dl_am_info->is_mute) {
            ull_dl_am_info->is_mute = is_mute;
            //bt_ull_le_am_play(mode, bt_ull_le_srv_get_codec_type(), false);
