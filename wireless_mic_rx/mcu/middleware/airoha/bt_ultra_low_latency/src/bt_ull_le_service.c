...
#define BT_ULL_LE_NVKEY_CSIS_DATA_LEN          16
...
bt_ull_le_srv_link_info_t g_ull_le_link_info[BT_ULL_LE_MAX_LINK_NUM];
...
#ifdef BT_ULL_LE_THROUGHPUT_DEBUG
uint32_t g_gpt_clock_start = 0;
uint32_t g_gpt_clock_end = 0;
uint32_t g_max_frame_seq = 0;
#endif
...
static void bt_ull_le_srv_set_transmitter_is_start(bt_ull_transmitter_t transmitter_type, bool is_start);
...
static bt_status_t bt_ull_le_srv_get_stream_by_transmitter(bt_ull_transmitter_t transmitter, bt_ull_streaming_t *streaming);
...
static void *bt_ull_le_srv_get_stream_info(bt_ull_role_t role, bt_ull_transmitter_t transmitter)
{
    bt_ull_le_srv_stream_context_t *stream_ctx = bt_ull_le_srv_get_stream_context();
...
static bt_ull_transmitter_t bt_ull_le_srv_get_transmitter_by_stream_interface(bt_ull_streaming_t *streaming)
{
    uint8_t i;
    for (i = 0; i < BT_ULL_TRANSMITTER_MAX_NUM; i++) {
...
static bt_ull_le_stream_port_mask_t bt_ull_le_srv_get_stream_port_by_transmitter(bt_ull_transmitter_t transmitter)
{
    if (transmitter < BT_ULL_TRANSMITTER_MAX_NUM) {
...
static bt_ull_transmitter_t bt_ull_le_srv_get_transmitter_by_stream_port(bt_ull_le_stream_port_mask_t stream_port)
{
    uint8_t i;
    for (i = 0; i < BT_ULL_TRANSMITTER_MAX_NUM; i++) {
...
static bt_ull_le_srv_audio_out_t bt_ull_le_srv_get_audio_out_by_stream_port(bt_ull_le_stream_port_mask_t stream_port)
{
    uint8_t i;
    for (i = 0; i < BT_ULL_TRANSMITTER_MAX_NUM; i++) {
        if (g_ull_le_port_trans_mapping_table[i].streaming_port == stream_port) {
...
static bool bt_ull_le_srv_is_any_streaming_started(void)
{
    bt_ull_le_srv_context_t *ctx = bt_ull_le_srv_get_context();
    bt_ull_le_srv_stream_context_t *stream_ctx = bt_ull_le_srv_get_stream_context();
    ...
    } else if ((BT_ULL_ROLE_CLIENT == ctx->role) && ((stream_ctx->client.dl.is_am_open) ||
    ...
    switch (event) {
        case BT_ULL_LE_CONN_SRV_EVENT_AIR_CIG_PARAMS_SET_DONE_IND: {
            ...
            if (BT_STATUS_SUCCESS == set_cig_ind->status) {
                ...
                if (BT_ULL_LE_SRV_STREAM_STATE_SET_CIG_PARAMETER == ctx->curr_stream_state) {
                    if (!bt_ull_le_srv_is_any_streaming_started()) {
        ...
        case BT_ULL_LE_CONN_SRV_EVENT_AIR_CIS_ESTABLISHED_IND: {
            ...
            if (link_info) {
                ...
                if (!bt_ull_le_srv_is_any_streaming_started() && BT_ULL_ROLE_SERVER == ctx->role) {
                    ctx->curr_stream_state = BT_ULL_LE_SRV_STREAM_STATE_IDLE;
        ...
        case BT_ULL_LE_CONN_SRV_EVENT_AIR_CIS_DESTROIED_IND: {
            ...
            ull_report("[ULL][LE] AIRCIS DISCONNECTED IND (Server), streaming:%d, allow_play: %d, switch_latency: %d, aud_qul_change: %d, cig_created: %d!, reason: %x, power_state: %x", 7,
                bt_ull_le_srv_is_any_streaming_started(),
        ...
        case BT_ULL_LE_CONN_SRV_EVENT_AIR_CIG_PARAMS_REMOVED_IND: {
            ...
            if (BT_STATUS_SUCCESS == cis_remove_ind->status) {
             ...
             #if 0
                if ((BT_ULL_ROLE_SERVER == ctx->role) && (ctx->is_share_buff_set) && (!bt_ull_le_srv_is_any_streaming_started())) {
                ...
                ctx->curr_stream_state = BT_ULL_LE_SRV_STREAM_STATE_IDLE;
                ull_report("[ULL][LE] AIRCIG Removed Success, curr_stream_state is 0x%x, %d, %d, %d, %d", 5, \
                    ctx->curr_stream_state,
                    bt_ull_le_srv_is_connected(),
                    stream_ctx->allow_play,
                    ctx->is_removing_cig_for_switch_latency,
                    ctx->is_removing_cig_for_change_aud_quality);
                if (bt_ull_le_srv_is_any_streaming_started() && bt_ull_le_srv_is_connected() && !stream_ctx->allow_play \
                    && !ctx->is_removing_cig_for_switch_latency
                    && !ctx->is_removing_cig_for_change_aud_quality) {
                    if (BT_STATUS_SUCCESS == bt_ull_le_conn_srv_set_air_cig_params(ctx->cis_num)) {
                        ctx->curr_stream_state = BT_ULL_LE_SRV_STREAM_STATE_SET_CIG_PARAMETER;
            ...
            if (ctx->is_removing_cig_for_switch_latency || ctx->is_removing_cig_for_change_aud_quality) {
        ...
        case BT_ULL_LE_CONN_SRV_EVENT_LABEL_CHANGED_IND: {
            ...
            ull_report("[ULL][LE] LABEL_CHANGED_IND, status is 0x%x, latency: %d, quality: %d", 3, \
                label_change->status, label_change->label_changed.latency, label_change->label_changed.audio_quility);
/*
        ...
        case BT_ULL_LE_CONN_SRV_EVENT_AIR_CIG_TABLE_SET_IND: {
            ...
            if (set_ind && (BT_STATUS_SUCCESS == set_ind->status)) {
...
static void bt_ull_le_srv_stop_audio_transmitter_callback(bt_ull_transmitter_t transmitter_type, bt_status_t status)
{
    ...
    //BTA-37783,when stop normal mode and enter special mode, not set is_transmitter_start flag, please refer to JIRA.
        ...
        if (!bt_ull_le_srv_is_any_streaming_started() && ctx->curr_stream_state >= BT_ULL_LE_SRV_STREAM_STATE_SET_CIG_PARAMETER) {/*all streaming are disabled.*/
        ...
        /*BTA-46167,timing issue,when stop audio transmitter finish, but check downlink port is in streaming,
        and stream_ctx->allow_play is BT_ULL_PLAY_ALLOW, and ull is connected, should start audio transmitter*/
        /*if (bt_ull_le_srv_is_streaming(BT_ULL_LE_STREAM_MODE_DOWNLINK) && (BT_ULL_PLAY_ALLOW == stream_ctx->allow_play) && bt_ull_le_srv_is_connected()) {
            bt_ull_le_at_start(transmitter_type, true);
        }*/
...
static void bt_ull_le_srv_start_audio_transmitter_callback(bt_ull_transmitter_t transmitter_type, bt_status_t status)
{
    ...
    if ((BT_STATUS_SUCCESS == status) && (BT_ULL_ROLE_SERVER == ctx->role)) {
        ...
        if (BT_ULL_PLAY_DISALLOW == stream_ctx->allow_play) {
            bt_ull_le_at_stop(transmitter_type, true);
        }
...
static void bt_ull_le_srv_stop_am_callback(bt_ull_le_am_mode_t mode, bt_status_t status)
{
    ...
    if ((BT_STATUS_SUCCESS == status) && (BT_ULL_ROLE_CLIENT == ctx->role)) {
        ...
        if (!bt_ull_le_srv_is_any_streaming_started()) {/*all streaming are disabled.*/
            /* disable controller and enable sniff mode after all streaming stop */
            ctx->curr_stream_state = BT_ULL_LE_SRV_STREAM_STATE_IDLE;
            /* TBD. enable link sniff policy */
        }
...
static bt_status_t bt_ull_le_srv_handle_sync_codec(uint16_t handle)
{
    ...
    if (BT_ULL_ROLE_SERVER == ull_context->role) {
        ...
        stream_info->audio_quality = stream_ctx->audio_quality;
        ...
        ull_report("[ULL][LE] sync headset/earbuds is allow_play: 0x%x, dl_latency:%d, ul_latency:%d, Mic_codec_sample_rate: 0x%x", 4,
            stream_info->allow_play, stream_info->dl_latency, stream_info->ul_latency, stream_info->server.stream[BT_ULL_MIC_TRANSMITTER].codec_param.sample_rate);
    }
...
static bt_status_t bt_ull_le_srv_handle_codec_sync_event(uint16_t handle, bt_ull_le_srv_stream_context_t *stream_info)
{
    ...
    if (BT_ULL_ROLE_SERVER == ctx->role) {
        ...
        if (BT_ULL_SPEAKER_CLIENT == ctx->client_type) {
            ...
            ull_report("[ULL][LE] bt_ull_le_srv_handle_codec_sync_event(aud_quality), %d, %d, %d, %d, %d, %d", 6, \
                change_aud_quality, stream_ctx->audio_quality, other_link_aud_quality, link_info->aud_quality, ctx->cs_size, ctx->curr_stream_state);
/*
...
*/
        ...
        if (!ctx->is_share_buff_set) {
            ...
            if ((BT_STATUS_SUCCESS == status) && (BT_STATUS_SUCCESS == bt_ull_le_srv_set_avm_share_buffer(ctx->client_type))) {
...
void bt_ull_le_srv_write_sirk_to_nvkey(bt_key_t *sirk)
{
    uint8_t data[BT_ULL_LE_NVKEY_CSIS_DATA_LEN] = {0};
    uint32_t size = BT_ULL_LE_NVKEY_CSIS_DATA_LEN;
    nvkey_status_t status = NVKEY_STATUS_OK;
    memcpy(data, (uint8_t *)sirk, sizeof(bt_key_t));
    status = nvkey_write_data(NVID_BT_LEA_CSIS_DATA, data, size);
    ull_report("[ULL][LE] bt_ull_le_srv_write_sirk_to_nvkey, status: %x, size : %d", 2, status, size);
}

bt_status_t bt_ull_le_srv_change_remote_sirk_handle(bt_ull_le_srv_change_remote_sirk_t *sirk)
{
    if (!sirk) {
        return BT_STATUS_FAIL;
    }
    ull_report("[ULL][LE] bt_ull_le_srv_change_remote_sirk_handle, sirk: %x-%x-%x...%x-%x-%x", 6, \
        sirk->sirk[0], sirk->sirk[1], sirk->sirk[2], sirk->sirk[13], sirk->sirk[14], sirk->sirk[15]);
    bt_key_t t_sirk = {0};
    bt_ull_le_srv_memcpy((uint8_t *)&t_sirk, (uint8_t *)sirk, sizeof(bt_key_t));
    bt_ull_le_srv_write_sirk_to_nvkey(&t_sirk);
    return bt_ull_le_srv_set_sirk(t_sirk);
}
        ...
        switch(req_data[0]) {
            ...
            case BT_ULL_EVENT_LATENCY_SWITCH_COMPLETE: {
                ...
                if (latency) {
                    ...
                    if ((bt_ull_le_srv_is_connected()) && bt_ull_le_srv_is_any_streaming_started()) {
            ...
            case BT_ULL_EVENT_SEND_CONFIGURATION: {
                ...
                ull_report("[ULL][LE] CONFIGURATION , audio_location: %d, cis_id: %d", 2, cfg->audio_location, bt_ull_le_conn_srv_get_cis_id_by_location(cfg->audio_location));
                if (BT_ULL_ROLE_CLIENT == ctx->role) {
...
*/
            ...
            case BT_ULL_EVENT_CALL_STATE: {
                ...
                void *data = req_data + sizeof(bt_ull_req_event_t) + sizeof(bt_ull_le_srv_call_status_t);
                bt_ull_le_call_srv_client_handle_call_state(call_state, data);
                break;
            }
            case BT_ULL_EVENT_CALL_ACTION: {
                bt_ull_le_srv_call_action_t call_action = req_data[1];
                void *data = req_data + sizeof(bt_ull_req_event_t) + sizeof(bt_ull_le_srv_call_action_t);
                bt_ull_le_call_srv_client_handle_call_action(call_action, data);
                break;
            }
            ...
            case BT_ULL_EVENT_CHNAGE_REMOTE_SIRK_IND: {
                bt_ull_le_srv_change_remote_sirk_t *sirk = (bt_ull_le_srv_change_remote_sirk_t *)(req_data + sizeof(bt_ull_req_event_t));
                bt_ull_le_srv_change_remote_sirk_handle(sirk);
                break;
            }
            default: {
...
bt_status_t bt_ull_le_srv_change_audio_quality_handle(bt_handle_t acl_handle, bt_ull_le_srv_audio_quality_t audio_quality)
{
    ...
    bt_ull_le_srv_context_t* ctx = bt_ull_le_srv_get_context();
    bt_ull_le_srv_stream_context_t *stream_ctx = bt_ull_le_srv_get_stream_context();
    ull_report("[ULL][LE] bt_ull_le_srv_change_audio_quality_handle, %d, %d, %d, %d, %d", 5, \
        acl_handle,
        audio_quality,
    ...
    if (BT_ULL_LE_SRV_AUDIO_QUALITY_TYPE_INVALID == audio_quality || BT_ULL_LE_SRV_AUDIO_QUALITY_HIGH_RESOLUTION < audio_quality) {
        return BT_STATUS_FAIL;
    }
    if (BT_ULL_ROLE_CLIENT == ctx->role) {
        if (audio_quality == stream_ctx->audio_quality) {
            return BT_STATUS_SUCCESS;
        }
        uint8_t i = BT_ULL_LE_CLIENT_LINK_MAX_NUM;
        while (0 != i) {
        ...
        if (!status) {
            stream_ctx->audio_quality = audio_quality;
        ...
        if ((BT_ULL_LE_SRV_AUDIO_QUALITY_HIGH_QUALITY == change_aud_quality && BT_ULL_LE_SRV_IS_SUPPORT(idx, BT_ULL_LE_SRV_FEATURE_MHDT_4M_PHY)) \
            ...
            //For Auto test script check log
            if (BT_ULL_LE_SRV_AUDIO_QUALITY_TYPE_DEFAULT == change_aud_quality) {
                ull_report_error("[ULL][LE] ULL2.0 Default Quality Enable.", 0);
            } else if (BT_ULL_LE_SRV_AUDIO_QUALITY_HIGH_QUALITY == change_aud_quality) {
                ull_report_error("[ULL][LE] ULL2.1 High-Quality Enable.", 0);
            } else if (BT_ULL_LE_SRV_AUDIO_QUALITY_HIGH_RESOLUTION == change_aud_quality) {
                ull_report_error("[ULL][LE] ULL2.1 High-Resolution Enable.", 0);
            }
        ...
        if (ctx->is_cig_created && !ctx->is_removing_cig_for_change_aud_quality) {
            //if (BT_STATUS_SUCCESS != bt_ull_le_conn_srv_change_audio_quality(audio_quality)) {
            status = bt_ull_le_conn_srv_remove_air_cig_params();
            if (BT_STATUS_SUCCESS == status || BT_STATUS_PENDING == status) {
...
void bt_ull_le_srv_lock_streaming(bool lock)
{
    ...
    if (BT_ULL_ROLE_SERVER == ctx->role) {
     ...
     if (lock) {
        ...
        bt_ull_le_srv_notify_server_play_is_allow(BT_ULL_PLAY_DISALLOW, BT_ULL_ALLOW_PLAY_REASON_FOTA);
      ...
      /* start current play & allow dongle play */
        ...
        bt_ull_le_srv_notify_server_play_is_allow(BT_ULL_PLAY_ALLOW, BT_ULL_ALLOW_PLAY_REASON_FOTA);
    }
  }
}
...
void bt_ull_le_srv_notify_server_play_is_allow(bt_ull_allow_play_t is_allow, uint8_t reason)
{
    ...
    bt_ull_le_srv_stream_context_t *stream_ctx = bt_ull_le_srv_get_stream_context();
    ull_report("[ULL][LE] bt_ull_le_srv_notify_server_play_is_allow, %d, %d, %d, %x, %d", 5, \
...
bt_status_t bt_ull_le_srv_init(bt_ull_role_t role, bt_ull_callback callback)
{
    ...
    if (BT_ULL_ROLE_CLIENT == g_ull_le_ctx.role) {
        ...
        codec_param->codec_type = bt_ull_le_srv_get_client_preffered_codec();
        ...
        g_ull_le_stream_ctx.codec_type = codec_param->codec_type;
#ifdef AIR_WIRELESS_MIC_ENABLE
        if (BT_ULL_LE_CODEC_ULD == g_ull_le_stream_ctx.codec_type) {
            bt_ull_le_srv_set_client_preferred_channel_mode(BT_ULL_LE_CHANNEL_MODE_MONO);
        }
#endif
...
bt_status_t bt_ull_le_srv_change_remote_sirk(bt_ull_change_remote_sirk_t *action)
{
    bt_status_t status = BT_STATUS_FAIL;
    uint8_t len = sizeof(bt_ull_req_event_t) + sizeof(bt_ull_le_srv_change_remote_sirk_t);
    uint8_t *ind = (uint8_t *)bt_ull_le_srv_memory_alloc(len);
    if (!ind || !action) {
        ull_report_error("[ULL][LE] bt_ull_le_srv_change_remote_sirk, OOM! %x, %x", 2, ind, action);
        return status;
    }
    bt_ull_le_srv_link_info_t *link = bt_ull_le_srv_get_link_info(action->handle);
    if (!link) {
        ull_report_error("[ULL][LE] bt_ull_le_srv_change_remote_sirk, not find handle : 0x%x", 1, action->handle);
        return status;
    }
    ull_report("[ULL][LE] bt_ull_le_srv_change_remote_sirk, handle: %d, state: %d, sirk: %x-%x-%x...%x-%x-%x", 8, \
        link->conn_handle,
        link->curr_state,
        action->sirk[0], action->sirk[1], action->sirk[2], action->sirk[13], action->sirk[14], action->sirk[15]);
    if (BT_ULL_LE_LINK_STATE_READY > link->curr_state) {
        return status;
    }
    ind[0] = BT_ULL_EVENT_CHNAGE_REMOTE_SIRK_IND;
    bt_ull_le_srv_change_remote_sirk_t *sirk = (bt_ull_le_srv_change_remote_sirk_t *)(ind + sizeof(bt_ull_req_event_t));
    bt_ull_le_srv_memcpy(sirk, (uint8_t *)&action->sirk, sizeof(bt_ull_le_srv_change_remote_sirk_t));
    status = bt_ull_le_srv_send_data(link->conn_handle, ind, len);
    bt_ull_le_srv_memory_free(ind);
    return status;
...
bt_status_t bt_ull_le_srv_action(bt_ull_action_t action, const void *param, uint32_t param_len)
{
    ...
    switch (action) {
        ...
        case BT_ULL_ACTION_CHANGE_REMOTE_SIRK: {
            ull_assert(param);
            bt_ull_change_remote_sirk_t *change_sirk = (bt_ull_change_remote_sirk_t *)param;
            bt_ull_le_srv_change_remote_sirk(change_sirk);
            break;
        }
        default:
            status = BT_STATUS_SUCCESS;
...
bt_status_t bt_ull_le_srv_active_streaming(void)
{
    ...
    uint8_t i;
    for (i = 0; i < max_link_count; i++) {
        ull_report("[ULL][LE] bt_ull_le_srv_active_streaming, link index: %d, handle: %x, state: %d", 3, i, g_ull_le_link_info[i].conn_handle, g_ull_le_link_info[i].curr_state);
...
bt_status_t bt_ull_le_srv_deactive_streaming(void)
{
    ...
    uint8_t i;
    for (i = 0; i < max_link_count; i++) {
        ull_report("[ULL][LE] bt_ull_le_srv_deactive_streaming, link index: %d, handle: %x, state: %d", 3, i, g_ull_le_link_info[i].conn_handle, g_ull_le_link_info[i].curr_state);
...
{
    ull_report("[ULL][LE][SD_DEBUG] bt_ull_le_srv_silence_detection_notify_client_status, is_silence: %d, transmitter_type: 0x%x", 2, is_silence, transmitter_type);
    if (bt_ull_le_srv_is_connected()) {
