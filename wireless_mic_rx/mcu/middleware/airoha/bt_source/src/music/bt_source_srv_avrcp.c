...
bt_status_t bt_source_srv_notify_avrcp_event_change(const bt_bd_addr_t *address, bt_avrcp_event_t event, void *data)
{
    ...
    LOG_MSGID_I(source_srv,"[AVRCP_SRV]notify_avrcp_event_change:state = 0x%x, event:%x ", 1, *state, event);
...
static uint32_t bt_source_srv_avrcp_get_volume_step(uint8_t avrcp_volume)
{
    ...
    float avrcp_step_f = (avrcp_level_f *50) / BT_AVRCP_VOL_IN_LEVEL_VALUE_MAX + 0.5f;
    LOG_MSGID_I(source_srv,"[AVRCP_SRV]avrcp_get_volume_step-avrcp_vol: %x, vol_step: %x", 2, avrcp_volume, (uint32_t)avrcp_step_f);
