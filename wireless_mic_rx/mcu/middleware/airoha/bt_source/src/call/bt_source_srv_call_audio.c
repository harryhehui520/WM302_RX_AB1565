...
static bt_source_srv_call_audio_sub_id_t bt_source_srv_call_audio_mapping_sub_id(bt_source_srv_call_audio_play_t play_type)
{
    switch (play_type) {
        case BT_SOURCE_SRV_CALL_AUDIO_PLAY_TYPE_UL: {
            return AUDIO_TRANSMITTER_BT_AUDIO_DONGLE_UL_HFP_USB_OUT_0;
        }
        break;
        case BT_SOURCE_SRV_CALL_AUDIO_PLAY_TYPE_DL: {
            ...
            if (bt_source_srv_common_audio_find_port_context(BT_SOURCE_SRV_PORT_CHAT_SPEAKER, &port_context) != BT_STATUS_SUCCESS) {
                if (bt_source_srv_common_audio_find_port_context(BT_SOURCE_SRV_PORT_GAMING_SPEAKER, &port_context) == BT_STATUS_SUCCESS) {
                    return AUDIO_TRANSMITTER_BT_AUDIO_DONGLE_DL_HFP_USB_IN_1;
                }
            } else {
                return AUDIO_TRANSMITTER_BT_AUDIO_DONGLE_DL_HFP_USB_IN_0;
            }
        }
        break;
        ...
        case BT_SOURCE_SRV_CALL_AUDIO_PLAY_TYPE_LINE_IN: {
            return AUDIO_TRANSMITTER_BT_AUDIO_DONGLE_DL_HFP_AFE_IN_0;
        }
        break;
        case BT_SOURCE_SRV_CALL_AUDIO_PLAY_TYPE_I2S_IN: {
            return AUDIO_TRANSMITTER_BT_AUDIO_DONGLE_DL_HFP_AFE_IN_1;
        }
        break;
#endif
        default:
    ...
    LOG_MSGID_E(source_srv, "[AG][AUDIO] mapping sub id fail by play type = %02x", 1, play_type);
    return BT_SOURCE_SRV_CALL_AUDIO_INVALID_SUB_ID;
}
...
bt_status_t bt_source_src_call_audio_mute(bt_source_srv_call_audio_play_t play_type)
{
    bt_source_srv_call_audio_sub_id_t sub_id = bt_source_srv_call_audio_mapping_sub_id(play_type);
    if (BT_SOURCE_SRV_CALL_AUDIO_INVALID_SUB_ID == sub_id) {
        return BT_STATUS_FAIL;
    }
    ...
    LOG_MSGID_I(source_srv, "[AG][AUDIO] audio play type = %02x mute status = %02x", 2, play_type, audio_status);
    return (audio_status == AUDIO_TRANSMITTER_STATUS_SUCCESS) ? BT_STATUS_SUCCESS : BT_STATUS_FAIL;
}

bt_status_t bt_source_src_call_audio_unmute(bt_source_srv_call_audio_play_t play_type)
{
    bt_source_srv_call_audio_sub_id_t sub_id = bt_source_srv_call_audio_mapping_sub_id(play_type);
    if (BT_SOURCE_SRV_CALL_AUDIO_INVALID_SUB_ID == sub_id) {
        return BT_STATUS_FAIL;
    ...
    LOG_MSGID_I(source_srv, "[AG][AUDIO] audio play type = %02x unmute status = %02x", 2, play_type, audio_status);
    return (audio_status == AUDIO_TRANSMITTER_STATUS_SUCCESS) ? BT_STATUS_SUCCESS : BT_STATUS_FAIL;
}
