...
#define RACE_NORMAL_INIT_CMD         (3)
#define RACE_NORMAL_DEINIT_CMD       (4)

race_port_t race_normal_get_active_port(void);
RACE_ERRCODE race_init_normal_port(race_port_t port);
...
#ifdef ARC_NXP_COM_ENABLE
#else /* !ARC_NXP_COM_ENABLE */
	else if (RACE_NORMAL_INIT_CMD == cmd) {
        port = (race_port_t)pmsg->dev_t;
        res = race_init_normal_port(port);
#if defined(MTK_ATCI_VIA_MUX) && defined(MTK_MUX_ENABLE)
        atci_mux_port_reinit(port, false, false);
#endif
        RACE_LOG_MSGID_I("race_normal_init, port[%d], res[%d]", 2, port, res);
    } else if (RACE_NORMAL_DEINIT_CMD == cmd) {
        port = race_normal_get_active_port();
        if (RACE_INVALID_PORT != port) {
            race_close_port_for_all_user(port);
        }
#if defined(MTK_ATCI_VIA_MUX) && defined(MTK_MUX_ENABLE)
        atci_mux_port_resume(false, false);
#endif
        RACE_LOG_MSGID_I("race_normal_deinit", 0);
    }
#endif /* ARC_NXP_COM_ENABLE */
}
...
#ifdef ARC_NXP_COM_ENABLE
#else /* !ARC_NXP_COM_ENABLE */
void race_normal_init(race_port_t port, uint32_t baudrate)
{
    race_general_msg_t msg_item = {0};
    msg_item.msg_id = MSG_ID_RACE_1WIRE_INIT_IND;
    msg_item.dev_t = (serial_port_dev_t)port;
    msg_item.msg_data = (uint8_t *)RACE_NORMAL_INIT_CMD;
    race_send_msg(&msg_item);
}

void race_normal_deinit(void)
{
    race_general_msg_t msg_item = {0};
    msg_item.msg_id = MSG_ID_RACE_1WIRE_INIT_IND;
    msg_item.msg_data = (uint8_t *)RACE_NORMAL_DEINIT_CMD;
    race_send_msg(&msg_item);
}

uint32_t race_normal_tx(uint32_t mux_handle, uint8_t *p_data, uint32_t len)
{
    mux_status_t mux_status = MUX_STATUS_ERROR;
    uint32_t data_len = 0;
    mux_buffer_t mux_buffer = {p_data, len};
    ...
    mux_status = mux_tx(mux_handle, &mux_buffer, 1, &data_len);
    RACE_LOG_MSGID_I("race, normal, send_data res:%d", 1, mux_status);
    if (MUX_STATUS_OK == mux_status) {
        return data_len;
    } else {
        return 0;
...
static void race_normal_mux_callback(mux_handle_t handle, mux_event_t event, uint32_t data_len, void *user_data)
{
    // HWJ: nothing.
}
...
static bool race_normal_prepare_open(race_port_t port)
{
    if (MUX_UART_BEGIN > port || MUX_UART_END < port) {
        return false;
    }
...
RACE_ERRCODE race_init_normal_port(race_port_t port)
{
    ...
    if (race_normal_prepare_open(port) == false) {
        res = RACE_ERRCODE_PARAMETER_ERROR;
        goto INIT_OVER;
    }
    step = 1;
    ...
    port_config.port = port;
    port_config.port_type = RACE_PORT_TYPE_1WIRE_UART;
    port_config.tx_function = race_normal_tx;
    ...
    user_config.port_type = RACE_PORT_TYPE_1WIRE_UART;
    user_config.user_name = NULL;
    user_config.mux_event_post_handler = race_normal_mux_callback;
    res = race_open_port(&user_config);

INIT_OVER:
    RACE_LOG_MSGID_I("race_init_normal_port, step:%d, port:%d, res:%d", 3, step, port, res);
    return res;
}
#endif /* ARC_NXP_COM_ENABLE */
