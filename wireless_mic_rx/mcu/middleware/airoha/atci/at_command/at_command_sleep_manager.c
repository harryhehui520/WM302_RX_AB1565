...
atci_status_t atci_cmd_hdlr_sleep_manager(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    if (strstr((char *)parse_cmd->string_ptr, "AT+SM=0") != NULL) {
        ...
        *SPM_DSP_WAKEUP_SOURCE_MASK = 0xFFFFFFFF;
        ...
        if (input_1 == 2) {
            *((volatile uint32_t *)HW_SYSRAM_DSP_PERFMON2_SELECT_START) = input_2;
            *((volatile uint32_t *)HW_SYSRAM_DSP_PERFMON2_MASK_START) = input_3;
            snprintf((char *)response->response_buf, sizeof(response->response_buf), "+DSP_PERFMON2:%d,0x%x,0x%x\r\n", input_1, *((volatile unsigned int *)HW_SYSRAM_DSP_PERFMON2_SELECT_START), *((volatile unsigned int *)HW_SYSRAM_DSP_PERFMON2_MASK_START));
        } else {
            *((volatile uint32_t *)HW_SYSRAM_DSP_PERFMON1_SELECT_START) = input_2;
            *((volatile uint32_t *)HW_SYSRAM_DSP_PERFMON1_MASK_START) = input_3;
            snprintf((char *)response->response_buf, sizeof(response->response_buf), "+DSP_PERFMON1:%d,0x%x,0x%x\r\n", input_1, *((volatile unsigned int *)HW_SYSRAM_DSP_PERFMON1_SELECT_START), *((volatile unsigned int *)HW_SYSRAM_DSP_PERFMON1_MASK_START));
        }
