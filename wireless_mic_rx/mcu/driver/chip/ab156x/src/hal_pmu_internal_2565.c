...
uint32_t pmu_get_register_value_lp(uint32_t address, uint32_t mask, uint32_t shift)
{
    ...
    if ((address == CHG_STATE_ADDR) && (mask == CHG_STATE_MASK) && (shift == CHG_STATE_SHIFT))
    {
        extern uint32_t battery_management_gauge_get_percent(void);
        if (battery_management_gauge_get_percent() >= 100)
        {
            log_hal_msgid_info("[PMU_BASIC]chg complete.", 0);
            return CHG_COMPL;
        }
    }
...
void hal_pmu_sleep_backup(void) {
    ...
    pmu_set_register_value_lp(LV3_STB_REGSEL_ADDR, LV3_STB_REGSEL_MASK, LV3_STB_REGSEL_SHIFT, 0xC);
}
