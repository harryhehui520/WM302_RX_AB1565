...
#if defined(AIR_SYSTEM_CLOCK_208M)
    static const uint32_t target_freq = 208000;
#elif defined(AIR_SYSTEM_CLOCK_104M)
    static const uint32_t target_freq = 104000;
#elif defined(AIR_SYSTEM_CLOCK_52M)
    static const uint32_t target_freq = 52000;
#else
#error "AIR_SYSTEM_CLOCK_XXX not defined"
#endif

#if defined(AIR_MAX_SYSTEM_CLOCK_208M)
    static const dvfs_frequency_t max_opp_idx = HAL_DVFS_HIGH_SPEED_208M;
#elif defined(AIR_MAX_SYSTEM_CLOCK_104M)
    static const dvfs_frequency_t max_opp_idx = HAL_DVFS_FULL_SPEED_104M;
#elif defined(AIR_MAX_SYSTEM_CLOCK_52M)
    static const dvfs_frequency_t max_opp_idx = HAL_DVFS_HALF_SPEED_52M;
#else
#error "AIR_MAX_SYSTEM_CLOCK_XXX not defined"
#endif
    ...
    #endif
#elif defined(AIR_SYSTEM_CLOCK_104M)
    #if defined(AIR_MAX_SYSTEM_CLOCK_52M)
        #error "AIR DVFS MAX/MIN Level Define Conflict"
    #endif
#endif
...
hal_dvfs_status_t hal_dvfs_lock_control(dvfs_frequency_t freq,hal_dvfs_lock_parameter_t lock) {
    ...
    if (freq != HAL_DVFS_CURR_SPEED) {
        if(freq > max_opp_idx) {
            log_hal_msgid_error("[DVFS] lock_control: Invalid freq %d, Lock %d, Caller 0x%08X", 3, freq, lock, xLinkRegAddr);
