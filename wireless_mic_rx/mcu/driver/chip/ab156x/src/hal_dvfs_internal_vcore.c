...
#else
static uint32_t dvfs_vcore_frequency[HAL_DVFS_MAX_SPEED] = { DVFS_VCORE_MODE_LOW_SPEED };
#endif
...
ATTR_RWDATA_IN_TCM static dvfs_opp_t domain={
    ...
    .basic_opp_index = HAL_DVFS_HALF_SPEED_52M,
#else
...
#endif
    uint32_t lposc_khz = hal_clock_get_lposc_khz();
    domain.frequency[HAL_DVFS_HALF_SPEED_52M] = lposc_khz / (4 * 2);
    domain.frequency[HAL_DVFS_FULL_SPEED_104M] = lposc_khz / (2 * 2);
    domain.frequency[HAL_DVFS_HIGH_SPEED_208M] = lposc_khz / 2;
