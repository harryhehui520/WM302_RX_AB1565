...
static void audio_class_d_enable_handle()
{
    ...
    //hal_gpt_delay_us(500); /* dummy wait for BT, BT will discard first cali value, so not need delay */
...
#define     HAL_RTC_USER_STATE_MASK    0x00FFFFFF
...
void    hal_rtc_switch_to_dcxo(hal_rtc_clock_switch_user_t user_id, bool    enable_dcxo)
{
    ...
    if(enable_dcxo == true){
        if((s_user_enable_state & HAL_RTC_USER_STATE_MASK) == 0){
