...
#else  // LPOSC 399M for De-Sense (Degradation of Sensitivity)
ATTR_RWDATA_IN_TCM static cali_info_t osc_tune_tbl[TUNE_TBL_SIZE] = {{.scale = lposc_cali_scale, .src = AD_OSC_D3_CK,.cali_target = OSC_CLK_133M},
                                                                     {.scale = lposc_ft_scale,   .src = AD_OSC_CK,   .cali_target = OSC_CLK_399M}};
#endif
...
#else
  #ifdef FPGA_ENV
    uint8_t curr_sel_num = 0;
  #else
    uint8_t curr_sel_num = CKSYS_CLK_CFG_REG.CLK_SYS_SEL;
  #endif
    
    uint32_t curr_cpu_freq = 0, lposc_hz = hal_clock_get_lposc_hz();

    switch(curr_sel_num) {
        case 0:
            curr_cpu_freq = 26000000 / 2;
            break;
        case 1:
            curr_cpu_freq = lposc_hz / (16 * 2);
            break;
        case 2:
            curr_cpu_freq = lposc_hz / 2;
            break;
        case 3:
            curr_cpu_freq = lposc_hz / (2 * 2);
            break;
        case 4:
            curr_cpu_freq = lposc_hz / (4 * 2);
            break;
        case 5:
            curr_cpu_freq = lposc_hz / (5 * 2);
            break;
        case 6:
            curr_cpu_freq = 156000000 / 2;
            break;
        case 7:
            curr_cpu_freq = 104000000 / 2;
            break;
        default:
            break;
    }
    return curr_cpu_freq;
#endif
}
...
uint32_t hal_clock_get_lposc_khz(void)
{
    return osc_tune_tbl[1].cali_target;
}

uint32_t hal_clock_get_lposc_hz(void)
{
    return(hal_clock_get_lposc_khz() * 1000);
}
