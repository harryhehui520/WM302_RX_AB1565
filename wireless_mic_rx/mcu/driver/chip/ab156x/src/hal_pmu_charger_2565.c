...
static bool chg_disable_forced = FALSE;

void pmu_chg_disable_forced(uint8_t disable)
{
    log_hal_msgid_error("[PMU_CHG]pmu_chg_disable_forced disable %d!", 1, disable);
    chg_disable_forced = disable;
}
...
#ifdef ARC_USB_SW_ENABLE
#else /* !ARC_USB_SW_ENABLE */
            /* HWJ: Disabled by forced. */
            if (chg_disable_forced)
            {
                log_hal_msgid_error("[PMU_CHG]enable_charger, chg_disable_forced!", 0);
                return 0;
            }
#endif /* ARC_USB_SW_ENABLE */
...
uint32_t pmu_ntc_get_ratio(void)
{
    int temp = 0;
    uint32_t temp_adc = 0, ratio = 0;
...
void pmu_ntc_update_state(int *ret_temp, pmu_ntc_state_t *curr_state, uint32_t *ret_ratio)
{
    ...
    uint32_t ratio = pmu_ntc_get_ratio();
...
{
    if (pmu_get_nvkey(NVID_CAL_VBAT_VOLT_CFG, (uint8_t *)&pmu_vbat_volt, sizeof(pmu_vbat_volt)) != PMU_STATUS_SUCCESS) {
        log_hal_msgid_error("[PMU_CHG]bat_init, fail", 0);
    }
