...
bool hal_audio_volume_set_digital_gain_fade_time_setting(hal_audio_volume_digital_gain_fade_time_setting_parameter_t *gain_fade_item_setting);
...
#if (HAL_AUDIO_HWGAIN_SET_FADE_TIME)
#include <math.h>
extern uint32_t afe_samplerate_convert_register_value_to_samplerate(afe_samplerate_general_t register_value);
extern afe_volume_digital_control_t afe_digital_gain[AFE_HW_DIGITAL_GAIN_NUM];
bool hal_audio_volume_set_digital_gain_fade_time_setting(hal_audio_volume_digital_gain_fade_time_setting_parameter_t *gain_fade_item_setting)
{
    afe_hardware_digital_gain_t gain_select         = hal_audio_hardware_gain_get_selcet(gain_fade_item_setting->memory_select);
    ...
    fade_time           = gain_fade_item_setting->fade_time;
    gain_index          = gain_fade_item_setting->gain_index;
    ...
    // g_gain_fade_item_setting[gain_select].fade_time = gain_fade_item_setting->fade_time;
    // g_gain_fade_item_setting[gain_select].memory_select = gain_fade_item_setting->memory_select;
    // g_gain_fade_item_setting[gain_select].gain_index = gain_fade_item_setting->gain_index;

    HAL_AUDIO_LOG_INFO("DSP - HWGAIN fade, hal_audio_volume_set_digital_gain_fade_time_setting(): HWGain%d, current_target_gain:0x%x, current_gain:0x%x, sample_rate:%d, agent_count:%d",
    ...
    if((current_target_gain != 0xFFFFFFFF) && (current_gain != 0xFFFFFFFF)){
        switch(gain_select){
            ...
            case AFE_HW_DIGITAL_GAIN3:
                ...
                break;
            default:
                AUDIO_ASSERT(0 && "DSP - HWGAIN Fade gain select error");
                break;
        }
        ...
        uint32_t current_gain_addr     = 0;
        uint32_t target_gain_addr      = 0;
        uint32_t down_step_addr        = 0;
        uint32_t up_step_addr          = 0;
        ...
        current_gain_addr = gain_con0_addr + 20;  // AFE_GAIN1_CUR;
        target_gain_addr  = gain_con0_addr + 4;   // AFE_GAIN1_CON1;
        down_step_addr    = gain_con0_addr + 8;   // AFE_GAIN1_CON2;
        up_step_addr      = gain_con0_addr + 12;  // AFE_GAIN1_CON3;
        ...
        //     samples_per_gain_step, sample_rate, (uint32_t)sample_rate_in_fp, fade_time);
        // Enter critical section to prevent invalid target gain value issue
        // which is caused by race condition
        HAL_AUDIO_ENTER_CRITICAL();
        if(AFE_READ(gain_con0_addr)&AFE_GAIN1_CON0_EN_MASK) {
            current_gain = AFE_READ(current_gain_addr);
            current_target_gain = AFE_READ(target_gain_addr);
            // Sync target gain to current gain to stop fade-in/out during register update
            AFE_SET_REG(target_gain_addr, current_gain, 0xfffff); // bit[0:19]
            // Uupdate samples_per_step
            AFE_SET_REG(gain_con0_addr, samples_per_gain_step << AFE_GAIN1_CON0_PER_STEP_POS, AFE_GAIN1_CON0_PER_STEP_MASK);
            // Set down_step and up_step
            AFE_SET_REG(down_step_addr, down_step, 0xfffff); // bit[0:19]
            AFE_SET_REG(up_step_addr, up_step, 0xfffff); // bit[0:19]
            // Restore target gain to resume fade-in/out
            AFE_SET_REG(target_gain_addr, current_target_gain, 0xfffff); // bit[0:19]
        }
        HAL_AUDIO_EXIT_CRITICAL();
        ...
        AUDIO_ASSERT(0 && "DSP - HWGAIN gain select wrong, plz check the input parameter");
    }
    return false;
}
