...
static void wired_audio_usb_out_open( mcu2dsp_open_param_p open_param, SOURCE source, SINK sink)
{
    ...
    config.resolution = RESOLUTION_16BIT;
    config.frame_size = 240 * sizeof(int16_t);
...
static void wired_audio_line_out_open(mcu2dsp_open_param_p open_param, SOURCE source, SINK sink)
{
    ...
    AUDIO_ASSERT(handle->src_fix_port == NULL);
    smp_config.channel_number = handle->out_channel_num;
    smp_config.in_sampling_rate = process_sample_rate;
    ...
    #if defined(AIR_FIXED_DL_SAMPLING_RATE_TO_96KHZ)
    AUDIO_ASSERT(handle->src_fix_2nd_port == NULL);
    smp_config.channel_number = handle->out_channel_num;
    smp_config.in_sampling_rate = 48000;
    smp_config.out_sampling_rate = 96000;
    smp_config.resolution = RESOLUTION_16BIT;
    smp_config.multi_cvt_mode = SRC_FIXED_RATIO_PORT_MUTI_CVT_MODE_CONSECUTIVE;
    smp_config.cvt_num = 2;
    smp_config.with_codec = false;
