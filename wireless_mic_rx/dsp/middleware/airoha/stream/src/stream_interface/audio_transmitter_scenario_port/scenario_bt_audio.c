...
#define BT_AUDIO_SAMPLE_RATE_BASE                      (8000) // 8K
...
static uint32_t bt_audio_dongle_dl_fs_convert_sample_rate_get(uint32_t in_rate, audio_dsp_codec_type_t codec_type, uint8_t sequence)
{
    ...
    if (codec_type == AUDIO_DSP_CODEC_TYPE_LHDC) {
        return bt_audio_usb_unified_sample_rate_setting_lhdc[raw_index][sequence];
    } else
#endif /* AIR_BT_AUDIO_DONGLE_LHDC_ENABLE */
    {
        return bt_audio_usb_unified_sample_rate_setting_common[raw_index][sequence];
    }
}
            ...
            switch (config_param->config_operation) {
                case BT_AUDIO_DONGLE_CONFIG_OP_SET_MUTE:
                    for (i = 0; i < source_info->usb_in.channel_num; i ++) {
                        stream_function_sw_gain_configure_gain_target(dongle_handle->gain_port, i, -144 * 100);;
                    }
                    BT_DONGLE_LOG_I("[BT Audio][DL][config] set mute, type %d", 1, source->scenario_type);
                    break;
                case BT_AUDIO_DONGLE_CONFIG_OP_SET_UNMUTE:
                    for (i = 0; i < source_info->usb_in.channel_num; i ++) {
                        stream_function_sw_gain_configure_gain_target(dongle_handle->gain_port, i, 0);
                    }
...
ATTR_TEXT_IN_RAM_FOR_MASK_IRQ static void bt_audio_dongle_dl_common_init(bt_audio_dongle_handle_t *dongle_handle)
{
    ...
    g_bt_audio_dongle_silence_detection_interval = bt_out->frame_interval;
...
#endif /* AIR_BT_AUDIO_DONGLE_LHDC_ENABLE */
    {
        ...
        frame_samples_base = dongle_handle->src_out_frame_samples / (sink_info->bt_out.sample_rate / BT_AUDIO_SAMPLE_RATE_BASE);
        sw_src_config.in_frame_size_max  = sw_src1_config.out_sampling_rate / BT_AUDIO_SAMPLE_RATE_BASE * frame_samples_base;
    ...
    bt_common_open_param_p bt_common_open_param)
{
    ...
    frame_samples_base                   = sink_info->bt_out.frame_interval * BT_AUDIO_SAMPLE_RATE_BASE / 1000 / 1000; // 8K base frame
    dongle_handle->src_out_frame_samples = sink_info->bt_out.sample_rate / BT_AUDIO_SAMPLE_RATE_BASE * frame_samples_base;
    dongle_handle->src_out_frame_size    = dongle_handle->src_out_frame_samples * format_bytes;
    dongle_handle->src_in_frame_samples  = source_info->usb_in.sample_rate / BT_AUDIO_SAMPLE_RATE_BASE * frame_samples_base;
