...
bool stream_function_audio_spectrum_meter_process(void *para)
{
    ...
    if (port->node && is_internal_buffer_data_ready) {
        ...
        for (uint32_t i = 0; i < ch; i++) {
            ...
            uint32_t temp_32bit_buf[240] = { 0 };
            if(port->resolution == RESOLUTION_16BIT){ //left shift 16bit
                for(uint32_t cnt = 0 ; cnt < (sizeof(temp_32bit_buf)/sizeof(uint32_t)) ; cnt++){
                    //DSP_MW_LOG_I("[Audio Spectrum Meter] 16bit %d",1,(sizeof(temp_32bit_buf)/sizeof(uint32_t)));
                    memcpy((uint8_t*)(&temp_32bit_buf[cnt]) + 2, input_buffer + cnt * 2, 2 * sizeof(uint8_t));
                    memcpy(input_buffer, temp_32bit_buf, 4 * sizeof(uint8_t));
                }
            }
