...
{
    if(is_Rx_only){
        g_cpd_reinit_mode = CPD_REINIT_MODE_SINGLE;
        if (VoRxCpdMemoryPtr) {
...
void voice_leq_send_update(int16_t pre_leq, int16_t post_leq)
{
    ...
    void *p_scratch = &VoRxCpdMemoryPtr->ScratchMemory[0];
        ...
        if (abs(pre_leq-post_leq)>3276) { //32768*0.1
#if AUDIO_CPD_DEBUG_LOG_ENABLE
        DSP_MW_LOG_I("[CPD] voice_leq_send_update gLEQUpdate %d, gLEQSend %d", 2,gLEQUpdate,gLEQSend);
#endif
            if ((gLEQUpdate == false) && (gLEQSend == true)) {
#if AUDIO_CPD_DEBUG_LOG_ENABLE
    ...
    if (gLEQUpdate == true) {
        ...
        gLEQSend = true;
#if AUDIO_CPD_DEBUG_LOG_ENABLE
        DSP_MW_LOG_I("[DSP][CPD][DSP send] gLEQ_gain%d, gLEQUpdate %d, gLEQSend %d",3,gLEQ_gain,gLEQUpdate,gLEQSend);
#endif
    }
}
...
bool Voice_CPD_Mode_Init(void *para, eCPD_VO_MODE vo_mode)
{
    ...
    gLEQUpdate = false;
    gLEQSend = true;
    DSP_MW_LOG_I("[CPD] initialize gLEQUpdate %d, gLEQSend %d", 2,gLEQUpdate,gLEQSend);
